...
for s in iller:
    ...
    title = f"{ad} Güncel Plaka Serisi ({AYLAR[today.month-1]} {today.year}) — Şu An Hangi Harf?"
    desc = f"{ad} güncel plaka: şu an tahmini {el} serisi veriliyor ({kod} plaka). Son doğrulanmış gözlem {kod} {ll} {ln:03d} ({tr_date(last_d)}). En son çıkan seri, harf geçişleri ve ileri tarih tahmini."
    page = f"""<!doctype html>
<html lang="tr">
<head>
...
<h1>{html.escape(ad)} güncel plaka serisi — şu an hangi seri veriliyor?</h1>
...
<div class="card">
<b>Sık sorulanlar</b>
<p class="meta" style="margin:10px 0 6px"><b>{html.escape(ad)} bugün hangi plaka serisinde?</b><br>Güncel tahminimiz <span class="mono">{kod} {el}</span> serisi. Kayıtlı en son gözlem {tr_date(last_d)} tarihli haftalık tabloda <span class="mono">{kod} {ll} {ln:03d}</span> idi; aradaki günler ilin olağan hızıyla ileri sarılır ve sayfa her hafta yeni tabloyla güncellenir.</p>
<p class="meta" style="margin:0 0 6px"><b>{html.escape(ad)} plakada bir harf serisi ne kadar dayanıyor?</b><br>Son aylarda haftada ~{hafta_seri:.1f} seri ilerliyor; yani 999 plakalık bir seri {sure} içinde tükeniyor.</p>
<p class="meta" style="margin:0"><b>Belirli bir plakaya ne zaman gelinir?</b><br><a href="/?il={s['kod']}">Tahmin aracına</a> istediğiniz seriyi yazın (örn. <span class="mono">{el[0]}ZZ 999</span>); verilmeyeceğini düşündüğünüz serileri de hesaptan çıkarabilirsiniz.</p>
</div>
